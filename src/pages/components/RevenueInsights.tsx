...
import { useGetSalesDataQuery, useGetWastageDataQuery, useGetsalesByRangeQuery, useGetSalesCostDataGraphQuery, useGetSalesByCategoryQuery, useGetSalesCostDataAnomalyGraphQuery } from '../../services/inventoryInsightsApi';
...
const ANOMALY_DATA = {
    ...
    anomalies: [null, null, null, null, null, null, null, 7200, null, null, null, 10800, null, null] as (number | null)[]
};
...
const CHART_CONFIGS = {
    ...
    currency: (customConfig?: any) => createChartOptions({
        tooltip: {
            callbacks: {
    ...
    percentage: (customConfig?: any) => createChartOptions({
        tooltip: {
            callbacks: {
...
// ===== REUSABLE COMPONENTS =====
const ChartContainer = ({ title, children, actions, height = "h-80" }: {
    title: string;
...
const MetricCard = ({ title, value, change, changeType, description, iconBg, iconColor }: MetricData) => {
    const changeColor = {
    ...
    return (
        <div className="bg-white rounded-xl p-5 pb-4 shadow-sm border border-gray-100 h-[180px] flex flex-col overflow-hidden transition-all duration-200 hover:shadow-md hover:-translate-y-1">
            ...
            <div className="flex-1 flex flex-col justify-start min-h-0 overflow-hidden">
                <div className="text-sm font-medium text-gray-600 mb-2 leading-tight line-clamp-2">{title}</div>
                <div className="text-2xl font-bold text-gray-900 mb-1.5 leading-tight flex-shrink-0">{value}</div>
                {description && (
                    <div className="text-xs text-gray-500 leading-tight line-clamp-2 flex-1">{description}</div>
                )}
            </div>
        </div>
    );
...
const SkeletonChartContainer = ({ title, height = "h-80" }: { title: string; height?: string }) => (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-gray-900">{title}</h3>
        </div>
...
const processSalesData = (data: any[]) => {
    ...
    const avgSales = actualSales.reduce((sum: number, val: number) => sum + val, 0) / actualSales.length;
...
const useProcessedSalesData = (salesCostData: any) => {
    return useProcessedSalesCostData(
        salesCostData,
        processSalesData,
        {
...
const extractCategoriesFromResponse = (categoryData: any): any[] => {
    ...
    if (categoryData?.data?.length && categoryData.data[0]?.daily_breakdowns?.length) {
...
const processCategoryData = (categoryData: any) => {
    ...
    const colors = [CHART_COLORS.primary, CHART_COLORS.success, CHART_COLORS.warning, CHART_COLORS.danger, CHART_COLORS.blue];

    return sortedCategories.map((cat: any, index: number) => ({
        label: cat.category || cat.name || `Category ${index + 1}`,
        percentage: parseFloat(cat.percentage_of_total || cat.percentage || (20 - index * 2)), // Fallback percentages
...
const processAnomalyData = (data: any[]) => {
    ...
    const average = salesValues.reduce((sum: number, val: number) => sum + val, 0) / salesValues.length;
...
const useMetricsData = (salesData: any, wastageData: any, salesByRangeData: any) => {
    ...
    return [
        { title: "Today's Sales", value: todaysSales, iconBg: "#DCFCE7", iconColor: CHART_COLORS.success },
        { title: "Waste on Sales", value: wastageCost, iconBg: "#FEE2E2", iconColor: CHART_COLORS.danger },
        { title: "Weekly Revenue", value: weeklyRevenue, iconBg: "rgba(124, 58, 237, 0.1)", iconColor: CHART_COLORS.purple },
        { title: "AI Forecast Accuracy", value: "94.2%", description: "Last 30 days average", iconBg: "#DBEAFE", iconColor: "#2563EB" }
    ];
};
...
const PieChart = ({ data }: { data: Array<{ label: string; percentage: number; color: string }> }) => (
...
const AnomalyChart = ({ data }: { data: any }) => {
    ...
    const revenueValues = data.revenue.filter((val: number) => val !== null && val !== undefined);
...
function RevenueInsights() {
    try {
        ...
        const { data: salesData, error: salesError, isLoading: salesLoading } = useGetSalesDataQuery({});
        const { data: wastageData, error: wastageError } = useGetWastageDataQuery({});
        const { data: salesByRangeData, error: rangeError } = useGetsalesByRangeQuery({});
        const { data: salesCostData, error: costError } = useGetSalesCostDataGraphQuery({});
        const { data: anomalyData, error: anomalyError } = useGetSalesCostDataAnomalyGraphQuery({});
        const { data: categoryData, error: categoryError, isLoading: categoryLoading } = useGetSalesByCategoryQuery({});

        // Handle errors silently - they're logged by RTK Query automatically

        // Processed data
        const metricsData = useMetricsData(salesData, wastageData, salesByRangeData);
            ...
            return (
                <>
                    ...
                    <div className="grid grid-cols-[2fr_1fr] gap-6 mb-10">
                        <SkeletonChartContainer title="Revenue   vs Forecast" height="h-[400px]" />
                        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                            <div className="flex justify-between items-center mb-4">
                    ...
                    <SkeletonChartContainer title="Revenue Anomaly Detection" height="h-[400px]" />
                </>
            );
        }
            ...
            <>
                ...
                <ChartContainer title="Revenue Anomaly Detection" height="h-[400px]">
                    <div className="bg-red-50 rounded-full px-3 py-1 inline-block text-xs font-medium text-red-800 mb-6">
                        {processedAnomalyData.anomalies.filter((a: any) => a !== null).length} Anomalies Detected
                    </div>
        ...
        return (
            <div className="flex items-center justify-center h-64 text-red-500">
                <div>
                    <h3 className="text-lg font-semibold mb-2">Error Loading Revenue Insights</h3>
