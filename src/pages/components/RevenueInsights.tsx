...
import { useGetSalesDataQuery, useGetWastageDataQuery, useGetsalesByRangeQuery, useGetSalesCostDataGraphQuery, useGetSalesByCategoryQuery, useGetSalesCostDataAnomalyGraphQuery, useGetSalesCostForecastQuery } from '../../services/inventoryInsightsApi';
...
const processSalesData = (data: any[], forecastData?: any[]) => {
    ...
    if (forecastData && forecastData.length > 0) {
        ...
        const lastDatePart = lastDataDate.includes('T') ? lastDataDate.split('T')[0] : lastDataDate;
...
const useMetricsData = (salesData: any, wastageData: any, salesByRangeData: any, isLoading: boolean = false): MetricData[] => {
    ...
    const chartOptions = createChartOptions({
        ...
        plugins: {
            ...
            afterDraw: (chart: any) => {
                ...
                const centerY = chartArea.top + (chartArea.bottom - chartArea.top) * 0.3;
...
function RevenueInsights() {
    try {
        ...
        const { data: forecastData, error: forecastError } = useGetSalesCostForecastQuery({});
        ...
        // Processed data
        const metricsData = useMetricsData(salesData, wastageData, salesByRangeData, isLoading);
        const processedSalesData = useProcessedSalesData(salesCostData, forecastData);
