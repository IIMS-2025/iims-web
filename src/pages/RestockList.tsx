...
export default function RestockList() {
    ...
    const bulkLoading = useSelector((state: RootState) => state.restock.bulkOperationsLoading);
    ...
    const loadRestockItems = async () => {
        try {
            ...
            dispatch(setError(error instanceof Error ? error.message : "Failed to load restock items"));
        }
    };
    ...
    const handleBulkPriorityUpdate = async (priority: RestockPriority) => {
        ...
        try {
            ...
            dispatch(setError(error instanceof Error ? error.message : "Failed to update priorities"));
        }
    };
    ...
    const handleBulkStatusUpdate = async (status: RestockStatus) => {
        ...
        try {
            ...
            dispatch(setError(error instanceof Error ? error.message : "Failed to update status"));
        }
    };

    const handleExportList = () => {
        const selectedData = filteredItems.filter(item => selectedItems.includes(item.id));
        const dataToExport = selectedData.length > 0 ? selectedData : filteredItems;

        // Create CSV content
        const headers = ["Item Name", "Category", "Current Stock", "Required Quantity", "Unit", "Priority", "Estimated Cost"];
        const csvContent = [
            headers.join(","),
                ...
                <>
                    ...
                    <Tabs
                        ...
                        rightContent={
                            <div className="tabs-right-content">
                                ...
                                >
                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                                        <path d="M8 10L5 7M8 10L11 7M8 10V2" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                ...
                                {sortedItems.map((item) => (
                                    <tr key={item.id} className="table-row">
                                        ...
                                        <td className="category-cell">
                                            <span className={`category-badge category-${item.category.toLowerCase().replace(/[^a-z]/g, '')}`}>
                                                {item.category}
                                            </span>
                                        </td>
                                        ...
                                        <td className="stock-cell">
                                            <div className="stock-info">
                                                <span className="quantity">{item.current_stock} {item.unit}</span>
                                            </div>
                                        </td>

                                        <td className="requirement-cell">
                                            <div className="requirement-info">
                                                <span className="requirement-quantity">{item.required_quantity} {item.unit}</span>
                                                <span className="requirement-period">Next {item.forecast_period_days} days</span>
                                            </div>
                                        </td>

                                        <td className="cost-cell">
                                            <div className="cost-info">
                                                <span className="total-cost">${item.estimated_cost.toFixed(2)}</span>
                                                <span className="unit-price">@${item.price_per_unit.toFixed(2)}/{item.unit}</span>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
                    ...
                    <div className="restock-pagination">
                        <div className="entries-info">
                            Showing 1 to {Math.min(sortedItems.length, 6)} of {stats.total} entries
                        </div>
